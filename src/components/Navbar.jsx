    ...
    <nav className="bg-gray-900 text-white shadow-lg sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16 items-center">
          ...
          {/* Desktop Menu */}
          <div className="hidden sm:flex items-center space-x-6 text-base font-medium">
          ...
          <div className="flex items-center space-x-4 relative">
            {user ? (
              <div className="relative group">
                <button className="flex items-center space-x-2 font-semibold text-sm text-indigo-300 hover:text-indigo-400 transition-colors px-3 py-1 rounded-lg hover:bg-gray-800">
                ...
                {/* Dropdown */}
                <div className="absolute right-0 mt-3 w-48 bg-gray-800 rounded-xl shadow-xl py-1 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-opacity duration-300 z-50 border border-gray-700">
                  <span className="block px-4 py-2 text-sm text-gray-400 border-b border-gray-700 truncate">{user.email}</span>
                  <button
                    onClick={handleLogout}
                    className="w-full text-left px-4 py-2 text-red-400 hover:bg-gray-700 transition-colors text-sm rounded-b-lg"
                  >
                    Logout
                  </button>
                </div>
      ...
      {/* Mobile Menu */}
      <div className={`sm:hidden bg-gray-800 border-t border-gray-700 transition-all duration-300 overflow-hidden ${menuOpen ? 'max-h-96 py-3' : 'max-h-0 py-0'}`}>
